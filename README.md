...
A persistent knowledge store for Claude Code, exposed as an MCP server. Claude remembers what it learns across sessions — gotchas, project context, technical knowledge — and retrieves it automatically via full-text search.
...
claude-brain is an [MCP server](https://modelcontextprotocol.io/) that runs as a subprocess of Claude Code. It communicates over stdio using the Model Context Protocol, giving Claude access to a SQLite database for persistent memory.
...
To get the most out of claude-brain, add instructions to your `CLAUDE.md` (global or per-project) telling Claude when and how to use it. For example:
...
```
cargo test --workspace     # Run all 30 tests
cargo build --workspace    # Debug build
cargo build --release      # Release build (produces target/release/claude-brain)
```
